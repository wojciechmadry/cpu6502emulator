...
enum class AVL_type { address, value, label, none };

enum class ADDITIONAL_info { LO, HI, none };

enum class VALUE_TYPE { binary = 2, decimal = 10, hex = 16 };

static_assert(static_cast<int>(Addressing::NUM_OF_ADDRESSING) == 13,
              "There are 13 addressing mode");
...
public:
  ...
  [[nodiscard]] const std::vector<SmallAddressInfo> &
  get_addressing_vector() const noexcept;
...
public:
  static AddressInfo
  create_address(Addressing mode, AVL_type avl = AVL_type::none,
                 std::string_view value = "",
                 VALUE_TYPE value_type = VALUE_TYPE::hex) noexcept;
