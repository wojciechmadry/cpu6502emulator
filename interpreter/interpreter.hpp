...
class Interpreter {
public:
  using INTERPRETER_CLONE_TYPE =
      std::pair<std::unordered_map<std::string, std::uint32_t>,
                CPU::CPU_CLONE_PAIR_TYPE>;
  ...
  std::optional<std::list<INTERPRETER_CLONE_TYPE>::iterator>
  get_current_state() const noexcept;
...
private:
  ...
  Addressing load_instruction(std::string_view address,
