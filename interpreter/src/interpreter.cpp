...
Interpreter::Interpreter(Interpreter &other) noexcept
    : m_cpu(other.m_cpu), m_labels(other.m_labels) {
  m_debug_actual_state = m_debug_states.end();
}
...
cpu6502::CPU &Interpreter::get_cpu() noexcept { return m_cpu; }

const cpu6502::CPU &Interpreter::get_cpu() const noexcept { return m_cpu; }

void Interpreter::add_label(std::string label_name,
...
Interpreter::get_labels() const noexcept {
  return m_labels;
}

void Interpreter::clear_labels() noexcept { m_labels.clear(); }
...
Addressing Interpreter::interprete(std::string_view line) {
  ...
  const auto instruction_info =
      cpu6502::interpreter::utils::get_instruction(line.substr(0, 3));
...
std::vector<std::string> &Interpreter::get_commands() { return m_asm_commands; }
