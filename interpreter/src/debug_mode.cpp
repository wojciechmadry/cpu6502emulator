...
std::uint32_t Interpreter::get_current_state_pose() const noexcept {
  ...
  for (auto it_begin = m_debug_states.begin(); it_begin != m_debug_actual_state;
       ++it_begin) {
    ++cur_pose;
  }
  return cur_pose;
...
const std::list<Interpreter::INTERPRETER_CLONE_TYPE> &
Interpreter::get_debug_states() const noexcept {
  return this->m_debug_states;
}

void Interpreter::debug_mode_change_command(
    utils::DebugModeNextCommand next_command) {
...
void Interpreter::load_state(
    std::list<INTERPRETER_CLONE_TYPE>::iterator it_state) {
...
void Interpreter::set_states_to_remember(
    std::uint32_t state_to_remember) noexcept {
...
std::optional<std::list<Interpreter::INTERPRETER_CLONE_TYPE>::iterator>
Interpreter::get_current_state() const noexcept {
