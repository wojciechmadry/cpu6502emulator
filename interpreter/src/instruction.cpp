...
const std::vector<InstructionInfo::SmallAddressInfo> &
InstructionInfo::get_addressing_vector() const noexcept {
...
AVL_type AddressInfo::get_avl() const noexcept { return m_type; }

void AddressInfo::set_avl(AVL_type type) noexcept { m_type = type; }
...
const std::string &AddressInfo::get_value() const noexcept { return m_value; }

void AddressInfo::set_value(std::string value) noexcept {
  m_value = std::move(value);
}

Addressing AddressInfo::get_mode() const noexcept { return m_mode; }

void AddressInfo::set_mode(Addressing mode) noexcept { m_mode = mode; }

VALUE_TYPE AddressInfo::get_value_type() const noexcept { return m_value_type; }
...
std::uint32_t AddressInfo::get_val_as_uint32() const {
  return static_cast<std::uint32_t>(
      std::stoul(m_value, nullptr, static_cast<int>(m_value_type)));
}

AddressInfo AddressInfo::create_address(Addressing mode, AVL_type avl,
