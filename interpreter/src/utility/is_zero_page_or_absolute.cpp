  ...
  if (dolar_position != std::string::npos) {
    ...
    if (!val.empty()) {
      ...
      if (iaddr != std::numeric_limits<std::uint32_t>::max()) {
        return {AddressInfo::create_address(Addressing::ZeroPage,
                                            AVL_type::address, val),
                AddressInfo::create_address(Addressing::Absolute,
                                            AVL_type::address, val)};
      }
    }
    ...
    return {AddressInfo::create_address(Addressing::ZeroPage, AVL_type::label,
                                        addr),
            AddressInfo::create_address(Addressing::Absolute, AVL_type::label,
                                        addr)};
  }
  return {};
}
