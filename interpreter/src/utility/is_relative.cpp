...
std::optional<AddressInfo> is_relative(std::string_view address) {
  if (is_propper_label(address)) {
    ...
    return AddressInfo::create_address(Addressing::Relative, AVL_type::label,
                                       std::move(addr));
  }
  ...
  if (found_star != std::string::npos) {
    ...
    if (plus_or_minus != '_') {
      ...
      if (!val.empty()) {
        return AddressInfo::create_address(
            Addressing::Relative, AVL_type::value,
            std::move(std::string{plus_or_minus} + val));
      }
    }
  }
  return {};
