...
is_zero_page_x_or_absolute_x(std::string_view address,
  ...
  if (X_found != std::string::npos || address.find('x') != std::string::npos) {
  ...
  if (dolar_position != std::string::npos && is_x_addressing) {
    ...
    if (!val.empty()) {
      ...
      if (iaddr != std::numeric_limits<std::uint32_t>::max()) {
        return {AddressInfo::create_address(Addressing::ZeroPageX,
                                            AVL_type::address, val),
                AddressInfo::create_address(Addressing::AbsoluteX,
                                            AVL_type::address, val)};
      }
    }
    ...
    return {AddressInfo::create_address(Addressing::ZeroPageX, AVL_type::label,
                                        addr),
            AddressInfo::create_address(Addressing::AbsoluteX, AVL_type::label,
                                        addr)};
  }
