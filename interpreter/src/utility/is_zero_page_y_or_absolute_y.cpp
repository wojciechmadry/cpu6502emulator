...
std::pair<std::optional<AddressInfo>, std::optional<AddressInfo>> is_zero_page_y_or_absolute_y(std::string_view address, std::size_t dolar_position, std::size_t comma_position)
{
    ...
    if(dolar_position != std::string::npos && is_y_addressing)
    {
        ...
        if(!val.empty())
        {
            ...
            {
                return {AddressInfo::create_address(Addressing::ZeroPageY, AVL_type::address, val), AddressInfo::create_address(Addressing::AbsoluteY, AVL_type::address, val)};
            }
        }
    }
    else if(is_propper_label(propably_label) && is_y_addressing)
    {
        ...
        return {AddressInfo::create_address(Addressing::ZeroPageY, AVL_type::label, addr), AddressInfo::create_address(Addressing::AbsoluteY, AVL_type::label, addr)};
    }
