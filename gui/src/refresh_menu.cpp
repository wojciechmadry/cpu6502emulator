...
void GuiManager::refreshMenu() {
  this->debugMode();
  this->label_debug_where_i_am->setText(
      QString::number(m_interpreter.get().get_current_state_pose()));
  ...
  // Load Carry Flag
  this->CF_value->setText(
      QString{boolToString(reg.PS.get(cpu6502::CPU::PSFlags::CarryFlag))});

  // Load Overflow Flag
  this->OF_value->setText(
      QString{boolToString(reg.PS.get(cpu6502::CPU::PSFlags::OverflowFlag))});

  // Load Negative Flag
  this->NF_value->setText(
      QString{boolToString(reg.PS.get(cpu6502::CPU::PSFlags::NegativeFlag))});

  // Load Interrupt Disable
  this->ID_value->setText(QString{
      boolToString(reg.PS.get(cpu6502::CPU::PSFlags::InterruptDisable))});

  // Load Decimal Mode
  this->DM_value->setText(
      QString{boolToString(reg.PS.get(cpu6502::CPU::PSFlags::DecimalMode))});

  // Load Break Command
  this->BC_value->setText(
      QString{boolToString(reg.PS.get(cpu6502::CPU::PSFlags::BreakCommand))});

  // Load Zero Flag
  this->ZF_value->setText(
      QString{boolToString(reg.PS.get(cpu6502::CPU::PSFlags::ZeroFlag))});
...
void GuiManager::showMemory() {
  ...
  auto setMemoryBox = [&](QGroupBox *group_box, QLineEdit *qline_edit,
                          std::uint32_t idx) {
