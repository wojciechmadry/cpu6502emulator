...
namespace CPU6502_TEST::inner{
    bool ROR_TEST()
    {
      bool all_good = true;
        ...
        opcode = static_cast<decltype(opcode)>(cpu6502::opcode::ROR::Accumulator); // 2 cycles
        ...
        for(cpu6502::Word i = 0u ; i <= 255u ; ++i)
        {
            ...
            const auto result = static_cast<cpu6502::Byte>((i >> 1) | (static_cast<int>(should_be_carry) << 7));
        ...
        {
            opcode = static_cast<decltype(opcode)>(cpu6502::opcode::ROR::ZeroPage); // 5 cycles
            cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
            mem[PC++] = opcode;
            mem[PC++] = 0x20;
        ...
        {
            opcode = static_cast<decltype(opcode)>(cpu6502::opcode::ROR::ZeroPageX); // 6 cycles
            cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
        ...
        // ASSERT ROR - Absolute
        {
            opcode = static_cast<decltype(opcode)>(cpu6502::opcode::ROR::Absolute); // 6 cycles
            cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
            mem[PC++] = opcode;
            mem[PC++] = 0x44;
            mem[PC++] = 0x44;
        ...
        {
            opcode = static_cast<decltype(opcode)>(cpu6502::opcode::ROR::AbsoluteX); // 7 cycles
            cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
            utils::load_to_xreg(cpu, 0x10);
            PC = cpu.get_registers().PC.get();
            mem[PC++] = opcode;
