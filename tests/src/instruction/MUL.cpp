...
bool MUL_TEST() {
  ...
  opcode = static_cast<decltype(opcode)>(
      cpu6502::opcode::MUL::Immediate); // 2 cycles
  ...
  for (std::int16_t i = -128; i < 127; ++i) {
    for (std::int16_t j = -128; j < 127; ++j) {
      ...
      all_good &=
          cpu.get_registers().ACU.get() == static_cast<cpu6502::Byte>(mul_res);
      all_good &= static_cast<cpu6502::SByte>(cpu.get_registers().ACU.get()) ==
                  static_cast<cpu6502::SByte>(mul_res);

      // Check overflow flag
      all_good &= cpu.get_registers().PS.get(PSFlags::OverflowFlag) ==
                  (mul_res > std::numeric_limits<cpu6502::SByte>::max() ||
                   mul_res < std::numeric_limits<cpu6502::SByte>::min());

      // Check Zero flag
      all_good &= cpu.get_registers().PS.get(PSFlags::ZeroFlag) ==
                  static_cast<bool>(static_cast<cpu6502::Byte>(mul_res) == 0);

      // Check Negative flag
      all_good &= cpu.get_registers().PS.get(PSFlags::NegativeFlag) ==
                  static_cast<bool>(mul_res & 0x80);
  ...
  opcode =
      static_cast<decltype(opcode)>(cpu6502::opcode::MUL::ZeroPage); // 3 Cycles
  ...
  all_good &=
      cpu.get_registers().ACU.get() == static_cast<cpu6502::Byte>(40 * 37);
  ...
  all_good &= cpu.get_registers().ACU.get() ==
              static_cast<cpu6502::Byte>(40 * 37 * 0x5);
  ...
  opcode = static_cast<decltype(opcode)>(
      cpu6502::opcode::MUL::ZeroPageX); // 4 Cycles
  ...
  all_good &=
      cpu.get_registers().ACU.get() == static_cast<cpu6502::Byte>(40 * 30);
  ...
  all_good &= cpu.get_registers().ACU.get() ==
              static_cast<cpu6502::Byte>(40 * 30 * 0x5);
  ...
  all_good &= cpu.get_registers().ACU.get() ==
              static_cast<cpu6502::Byte>(40 * 30 * 0x15);
  ...
  all_good &= cpu.get_registers().ACU.get() ==
              static_cast<cpu6502::Byte>(40 * 30 * 0x15 * 1);
  ...
  all_good &=
      cpu.get_registers().ACU.get() == static_cast<cpu6502::Byte>(40 * 15);
  ...
  all_good &=
      cpu.get_registers().ACU.get() == static_cast<cpu6502::Byte>(40 * 15 * 5);
  ...
  all_good &= cpu.get_registers().ACU.get() ==
              static_cast<cpu6502::Byte>(40 * 15 * 5 * 128);
  ...
  cpu.execute(6);
  all_good &=
      cpu.get_registers().ACU.get() == static_cast<cpu6502::Byte>(15 * 40);
  ...
  cpu.execute(6);
  all_good &=
      cpu.get_registers().ACU.get() == static_cast<cpu6502::Byte>(15 * 40 * 2);
  ...
  all_good &= cpu.get_registers().ACU.get() ==
              static_cast<cpu6502::Byte>(15 * 40 * 2 * 128);
