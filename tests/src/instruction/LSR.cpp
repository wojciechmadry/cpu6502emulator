...
bool LSR_TEST() {
  ...
  opcode = static_cast<decltype(opcode)>(
      cpu6502::opcode::LSR::Accumulator); // 2 cycles
  ...
  {
    opcode = static_cast<decltype(opcode)>(
        cpu6502::opcode::LSR::ZeroPage); // 5 cycles
    cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
    mem[PC++] = opcode;
    mem[PC++] = 0x20;
  ...
  {
    opcode = static_cast<decltype(opcode)>(
        cpu6502::opcode::LSR::ZeroPageX); // 6 cycles
    cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
  ...
  {
    opcode = static_cast<decltype(opcode)>(
        cpu6502::opcode::LSR::Absolute); // 6 cycles
    cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
    mem[PC++] = opcode;
    mem[PC++] = 0x44;
    mem[PC++] = 0x44;
  ...
  {
    opcode = static_cast<decltype(opcode)>(
        cpu6502::opcode::LSR::AbsoluteX); // 7 cycles
    cpu.get_registers().PS.set(PSFlags::CarryFlag, false);
    utils::load_to_xreg(cpu, 0x10);
    PC = cpu.get_registers().PC.get();
    mem[PC++] = opcode;
