...
bool DEBUG_MODE_TEST() {
  ...
  auto add_test = [&]() { all_tests.emplace_back(true); };
  ...
  std::vector<std::string> rnd_commands{"php",   "nop",      "lsr a",
                                        "ror a", "lda #225", "tax"};
  ...
  std::list<cpu6502::interpreter::Interpreter ::INTERPRETER_CLONE_TYPE>
      all_states;
  ...
  // Test 2 - Go max left and right
  {
    ...
    for (std::uint32_t i = 0u; i <= X * 2u; ++i) {
      interp.debug_mode_change_command(
