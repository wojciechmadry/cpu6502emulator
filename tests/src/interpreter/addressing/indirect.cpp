...
bool ADDRESSING_INDIRECT_TEST() {
  ...
  constexpr std::uint32_t clear_carry_value =
      (MSB_clear << 8) | LSB_clear; // 0xA4F0
  ...
  auto run_test = [&](const std::string &command, bool carry_value) {
    ...
    all_good &=
        all_reg.PS.get(cpu6502::registers::ProcessorStatus::Flags::CarryFlag) ==
        carry_value;
  };
  ...
  auto reset_position = [&]() {
    ...
    mem[clear_address] =
        static_cast<cpu6502::Byte>(cpu6502::opcode::CLC::Implied);
    mem[clear_address + 1] = 0xFF;
    mem[set_address] =
        static_cast<cpu6502::Byte>(cpu6502::opcode::SEC::Implied);
